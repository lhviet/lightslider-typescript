...
$.fn.extend({
  lightSlider: function (options) {
    ...
    let $children = $el.children(),
      ...
      thumbWidth = 0,
      interval = null;
    const property = (settings.vertical === true) ? 'height' : 'width';
    ...
    refresh.chbreakpoint = function () {
      ...
      if (settings.responsive.length) {
        ...
        if (typeof resposiveObj !== 'undefined' && resposiveObj !== null) {
          for (const j in resposiveObj.settings) {
    ...
    refresh.calWidth = function (cln) {
      const ln = cln === true ? $slide.find('.lslide').length : $children.length;
      if (settings.autoWidth === false) {
    ...
    plugin = {
      doCss: function () {
        const support = function () {
          const transition = ['transition', 'MozTransition', 'WebkitTransition',
            'OTransition', 'msTransition', 'KhtmlTransition'];
      ...
      keyPress: function () {
        if (settings.keyPress) {
          $(document).on('keyup.lightslider', function (e: any) {
      ...
      controls: function () {
        if (settings.controls) {
          $el.after('<div class="lSAction"><a class="lSPrev">' + settings.prevHtml
            + '</a><a class="lSNext">' + settings.nextHtml + '</a></div>');
        ...
        if (settings.loop === true && settings.mode === 'slide') {
          ...
          refresh.clone = function () {
            if (refresh.calWidth(true) > elSize) {
              ...
              const tItem = settings.autoWidth === true ? tI : settings.item;
      ...
      pager: function () {
        ...
        refresh.createPager = function () {
          ...
          const $theChildren = $slide.find('.lslide');
          const theLength = $slide.find('.lslide').length;
          let i = 0,
            pagers = '',
            v = 0;
          for (i = 0; i < theLength; i++) {
            ...
            const thumb = $theChildren.eq(i * settings.slideMove).attr('data-thumb');
            if (settings.gallery === true) {
              pagers += '<li style="width:100%;' + property + ':' + thumbWidth + 'px;'
                + gutter + ':' + settings.thumbMargin + 'px"><a href="#"><img src="' + thumb + '" /></a></li>';
            }
            else {
          ...
          const $pager = $cSouter.find('.lSPager').find('li');
        ...
        if (settings.pager) {
          ...
          const gMargin = (settings.vertical) ? 'margin-left' : 'margin-top';
      ...
      move: function (ob, v) {
        ...
        const $thumb = $slide.parent().find('.lSPager').find('li');
      ...
      slideThumb: function () {
        ...
        const $pager = $slide.parent().find('.lSPager');
        ...
        if (settings.mode === 'slide') {
          ...
          if ((swipeVal) >= w - elSize - settings.slideMargin) {
            if (settings.freeMove === false) {
              ...
              const swipeValT = w - elSize - settings.slideMargin;
      ...
      touchEnd: function (distance) {
        ...
        if (settings.mode === 'slide') {
          ...
          const gC = function (next) {
            ...
            if (!settings.autoWidth) {
              const num = slideValue / ((slideWidth + settings.slideMargin) * settings.slideMove);
              scene = num + ad;
              if (slideValue >= (w - elSize - settings.slideMargin)) {
                if (num % 1 !== 0) {
                  scene++;
          ...
          $(window).on('mouseup', function (e) {
            if (isDraging) {
              ...
              if (Math.abs(distance) >= settings.swipeThreshold) {
                $(window).on('click.ls', function (theEvent: any) {
    ...
    $el.goToNextSlide = function () {
      let nextI = true;
      if (settings.mode === 'slide') {
        const _slideValue = plugin.slideValue();
    ...
    $el.getCurrentSlideCount = function () {
      let sc = scene;
      if (settings.loop) {
        const ln = $slide.find('.lslide').length,
    ...
    $(window).on('resize orientationchange', function (e: any) {
      setTimeout(function () {
        if (e.preventDefault) {
          e.preventDefault();
        }
